...
FAKE_CHATGPT_PLAINTEXT_ONLY_COMPOSER_HTML = textwrap.dedent(
    """
    <!doctype html>
    <html>
      <body>
...
def test_post_ask_e2e_supports_plaintext_only_prompt_textarea_selector():
    fake_chatgpt_port = _free_port()
    fake_chatgpt_url = f"http://127.0.0.1:{fake_chatgpt_port}/"

    fake_chatgpt_server = Process(
        target=_run_fake_chatgpt_server,
        args=(fake_chatgpt_port, FAKE_CHATGPT_PLAINTEXT_ONLY_COMPOSER_HTML),
        daemon=True,
    )
    fake_chatgpt_server.start()
